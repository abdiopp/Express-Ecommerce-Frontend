...
const HomePage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `https://frail-coat-colt.cyclic.app/api/v1/product/product-list/${page}`
      );
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        "https://frail-coat-colt.cyclic.app/api/v1/product/product-count"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post(
        "https://frail-coat-colt.cyclic.app/api/v1/product/product-filters",
        {
          checked,
          radio,
        }
